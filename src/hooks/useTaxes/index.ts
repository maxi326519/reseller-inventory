      ...
      for (let i = 1; i <= 12; i++) {
        ...
        report.months[i].expenses.forEach((expense) => {
          ...
          if (expense.category === "COGS") {
            currentTaxes.month[i].expenses.COGS += expense.price;
          } else if (expense.category === "Ship Label") {
            currentTaxes.month[i].expenses.shipLabel += expense.price;
          } else if (expense.category === "Ebay Fees") {
            currentTaxes.month[i].expenses.ebayFees += expense.price;
          } else if (expense.category === "Ads Fee") {
            currentTaxes.month[i].expenses.adsFee += expense.price;
          } else {
            currentTaxes.month[i].expenses.otherExpenses += expense.price;

            // Check if the category already exist, else create them
            const name = toCamelCase(expense.category);
            let category = currentTaxes.month[i].expenses.otherCategories.find((category) => category.category === name);
            ...
            if (category) {
              ...
              currentTaxes.month[i].expenses.otherCategories.push({
          ...
          // Add price to total
          currentTaxes.month[i].expenses.total += expense.price;
        });
      }
